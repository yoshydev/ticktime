...
タイマー計測 → 日次〆処理（確定・履歴ロック・報告URL生成）→ 翌日再開、が基本サイクル。
...
- 純関数（`src/lib/workdate.ts` / `duration.ts` / `url.ts` / `template.ts` / `copyTemplates.ts`、`server/reportUrl.ts`、`closings.ts` の `computeInitialFinalSeconds`）にはvitestテストを付ける
...
- **報告URL**: settings `report_url_template` の `{変数}` を `encodeURIComponent` で埋め込んで生成（`server/reportUrl.ts`）。テンプレート空 = 機能無効（`form_url` は `''` を保存）。作業時間 `{hours}` は1時間単位で四捨五入した整数
- **コピー用テンプレート**: settings `copy_templates`（JSON配列 `[{label, template}]`、変数 `{ticket_key} {title}`）。0件でボタン非表示
...
- リモート: `origin`（git@github.com:yoshydev/ticktime.git）。コミット後のpushはユーザーの指示があった場合のみ
- pre-commitフックでgitleaksによるシークレットスキャンが走る（`.githooks/pre-commit`、`npm install` 時に `core.hooksPath` が自動設定）
